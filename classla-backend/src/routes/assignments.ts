...
import { requireCoursePermission, getCoursePermissions, getUserCourseRole } from '../middleware/authorization';
    ...
    const filterBlocks = (blocks: any[]): any[] => {
      ...
      return blocks
        ...
        .map(block => {
          ...
          if (block.attrs) {
            const { autograderData, correctAnswer, graderNotes, ...filteredAttrs } = block.attrs;
 ...
 */
const checkLockdownAccess = (assignment: Assignment, userId: string): { canAccess: boolean; message?: string } => {
 ...
 */
router.get('/assignment/:id/student', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
      ...
      if (!lockdownCheck.canAccess) {
        res.status(403).json({
          error: {
            ...
            message: lockdownCheck.message || 'Assignment is in lockdown mode',
            timestamp: new Date().toISOString(),
            path: req.path
          }
        });
 ...
 * Requirements: 3.5, 7.2
 */
router.get('/assignment/:id', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { id: userId, isAdmin } = req.user!;
    ...
    if (userRole === UserRole.INSTRUCTOR || userRole === UserRole.TEACHING_ASSISTANT || isAdmin) {
      res.json(assignment);
    } else {
...
router.post('/assignment', 
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (published_to && Array.isArray(published_to) && published_to.length > 0) {
        ...
        const invalidSections = sections?.filter(section => section.course_id !== course_id);
...
router.put('/assignment/:id', 
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const permissions = await getCoursePermissions(userId, existingAssignment.course_id, isAdmin);
      ...
      if (published_to && Array.isArray(published_to) && published_to.length > 0) {
        ...
        // Check if all sections belong to the same course
        const invalidSections = sections?.filter(section => section.course_id !== existingAssignment.course_id);
        if (invalidSections && invalidSections.length > 0) {
          res.status(400).json({
            error: {
              code: 'INVALID_SECTIONS',
      ...
      if (lockdown_time_map !== undefined) updateData.lockdown_time_map = lockdown_time_map;
 ...
 */
router.get('/assignment/:id/due-date/:userId', 
  authenticateToken, 
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Check permissions
      const permissions = await getCoursePermissions(currentUserId, assignment.course_id, isAdmin);
 ...
 * Requirements: 3.3, 3.4
 */
router.put('/assignment/:id/due-date/:userId', 
  authenticateToken, 
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { id, userId: targetUserId } = req.params;
      ...
      res.json({
        ...
        message: due_date ? 'Due date set successfully' : 'Due date removed successfully'
      });
    } catch (error) {
 ...
 * Requirements: 3.3, 3.4
 */
router.get('/assignment/:id/lockdown-status/:userId', 
  authenticateToken, 
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { id, userId: targetUserId } = req.params;
 ...
 * Requirements: 3.3, 3.4
 */
router.put('/assignment/:id/lockdown-time/:userId', 
  authenticateToken, 
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { id, userId: targetUserId } = req.params;
      ...
      if (lockdown_time_seconds !== null && lockdown_time_seconds !== undefined) {
        if (typeof lockdown_time_seconds !== 'number' || lockdown_time_seconds < 0) {
          res.status(400).json({
            error: {
      ...
      if (lockdown_time_seconds === null || lockdown_time_seconds === undefined) {
        ...
        const currentTime = Math.floor(Date.now() / 1000);
        updatedLockdownTimeMap[targetUserId] = currentTime + lockdown_time_seconds;
      }
      ...
      res.json({
        ...
        message: lockdown_time_seconds ? 'Lockdown time set successfully' : 'Lockdown time removed successfully'
      });
    } catch (error) {
