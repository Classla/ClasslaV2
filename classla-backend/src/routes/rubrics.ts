...
import { CreateRubricSchemaRequest, CreateRubricRequest, UpdateRubricRequest } from '../types/api';
 ...
 */
const calculateRubricScore = (values: number[], items: RubricItem[]): number => {
...
const updateSubmissionGradeFromRubric = async (
  ...
  if (schema.use_for_grading) {
    ...
    const maxPossibleScore = schema.items.reduce((sum: number, item: RubricItem) => sum + item.points, 0);
    
    // Calculate percentage grade (0-100)
    const gradePercentage = maxPossibleScore > 0 ? (totalScore / maxPossibleScore) * 100 : 0;
 ...
 */
router.get('/rubric-schema/:assignmentId', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!permissions.canRead) {
      res.status(403).json({
        error: {
          ...
          message: 'Not authorized to access rubric schemas for this assignment',
          timestamp: new Date().toISOString(),
          path: req.path
        }
      });
 ...
 * Requirements: 8.1, 8.4
 */
router.post('/rubric-schema', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    const { 
      assignment_id, 
      title, 
    ...
    if (!assignment_id || !title || !items || !Array.isArray(items) || items.length === 0) {
      res.status(400).json({
        error: {
          code: 'MISSING_REQUIRED_FIELDS',
          message: 'assignment_id, title, and items array are required',
          timestamp: new Date().toISOString(),
          path: req.path
        }
      });
    ...
    for (const item of items) {
      if (!item.title || typeof item.points !== 'number' || item.points < 0) {
        res.status(400).json({
          error: {
            code: 'INVALID_RUBRIC_ITEMS',
            message: 'Each rubric item must have a title and non-negative points value',
            timestamp: new Date().toISOString(),
            path: req.path
          }
        });
    ...
    if (!permissions.canManage && !permissions.canGrade) {
      res.status(403).json({
        error: {
          code: 'INSUFFICIENT_PERMISSIONS',
          message: 'Not authorized to create rubric schemas for this assignment',
          timestamp: new Date().toISOString(),
          path: req.path
        }
      });
 ...
 * Requirements: 8.1, 8.4
 */
router.put('/rubric-schema/:id', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { 
      title, 
    ...
    if (use_for_grading !== undefined) updateData.use_for_grading = use_for_grading;
 ...
 * Requirements: 8.1, 8.4
 */
router.delete('/rubric-schema/:id', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { id: userId, isAdmin } = req.user!;
    ...
    if (!permissions.canManage && !permissions.canGrade) {
      res.status(403).json({
        error: {
          code: 'INSUFFICIENT_PERMISSIONS',
          message: 'Not authorized to delete rubric schemas for this assignment',
          timestamp: new Date().toISOString(),
          path: req.path
        }
      });
    ...
    if (rubricInstances && rubricInstances.length > 0) {
      res.status(409).json({
        error: {
          ...
          message: 'Cannot delete rubric schema that is being used by rubric instances',
          timestamp: new Date().toISOString(),
          path: req.path
        }
      });
 ...
 * Requirements: 8.2, 8.3, 8.5
 */
router.get('/rubric/:submissionId', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (submission.student_id !== userId && !permissions.canGrade && !permissions.canManage) {
      res.status(403).json({
        error: {
          code: 'ACCESS_DENIED',
          message: 'Can only access rubrics for own submissions or need grading permissions',
          timestamp: new Date().toISOString(),
          path: req.path
        }
      });
 ...
 * Requirements: 8.2, 8.3, 8.5
 */
router.post('/rubric', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    const { 
      submission_id, 
      rubric_schema_id, 
      values 
    }: CreateRubricRequest = req.body;
    const { id: userId, isAdmin } = req.user!;

    // Validate required fields
    if (!submission_id || !rubric_schema_id || !values || !Array.isArray(values)) {
      res.status(400).json({
        error: {
          code: 'MISSING_REQUIRED_FIELDS',
          message: 'submission_id, rubric_schema_id, and values array are required',
          timestamp: new Date().toISOString(),
          path: req.path
        }
      });
    ...
    if (!permissions.canGrade && !permissions.canManage) {
      res.status(403).json({
        error: {
          code: 'INSUFFICIENT_PERMISSIONS',
          message: 'Not authorized to apply rubrics to submissions in this course',
          timestamp: new Date().toISOString(),
          path: req.path
        }
      });
    ...
    if (rubricSchema.assignment_id !== submission.assignment_id) {
      res.status(400).json({
        error: {
          ...
          message: 'Rubric schema does not belong to the submission\'s assignment',
          timestamp: new Date().toISOString(),
          path: req.path
        }
      });
    ...
    if (existingRubric) {
      res.status(409).json({
        error: {
          code: 'RUBRIC_EXISTS',
          message: 'A rubric already exists for this submission. Use PUT to update it.',
          timestamp: new Date().toISOString(),
          path: req.path
        }
      });
    ...
    try {
      await updateSubmissionGradeFromRubric(submission_id, rubric_schema_id, values);
    } catch (gradeError) {
      console.error('Error updating submission grade from rubric:', gradeError);
 ...
 * Requirements: 8.2, 8.3, 8.5
 */
router.put('/rubric/:id', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
 ...
 * Requirements: 8.2, 8.3, 8.5
 */
router.get('/rubric/:submissionId/score', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    const { submissionId } = req.params;
    const { id: userId, isAdmin } = req.user!;
    ...
    if (submission.student_id !== userId && !permissions.canGrade && !permissions.canManage) {
      res.status(403).json({
        error: {
          code: 'ACCESS_DENIED',
          message: 'Can only access rubric scores for own submissions or need grading permissions',
          timestamp: new Date().toISOString(),
          path: req.path
        }
      });
    ...
    const itemScores = rubric.values.map((value: number, index: number) => ({
      ...
      percentage: schema.items[index].points > 0 ? (value / schema.items[index].points) * 100 : 0
    }));
