...
describe("SignUpPage", () => {
  ...
  it("should render sign up page correctly", async () => {
    ...
    expect(
      screen.getByText(
        "Create your account to get started with Classla. Click the button below to sign up."
      )
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Sign Up with WorkOS" })
    ).toBeInTheDocument();
  ...
  it("should handle sign up error without message", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Failed to initiate sign up. Please try again.")
      ).toBeInTheDocument();
    });
  });
  ...
  it("should redirect to home when user is already authenticated", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.queryByText("Sign Up for Classla")).not.toBeInTheDocument();
    });
  });
  ...
  it("should disable button during loading", async () => {
    mockAuthService.redirectToSignup.mockImplementation(
      () => new Promise(() => {})
    ); // Never resolves
